"""Contract tests for agent + Rust alchemy boundary.
...
def test_agent_error_message_uses_snake_case_usage_keys() -> None:
    async def _scenario() -> None:
        async def failing_stream_fn(
            model: Model,
            context: Context,
            options: SimpleStreamOptions,
        ) -> FakeStreamResponse:
        ...
        agent = Agent(AgentOptions(stream_fn=failing_stream_fn))
        agent.set_model(
            Model(
                provider="openrouter",
                id="moonshotai/kimi-k2.5",
        ...
        assert "cacheRead" not in usage
        assert "cacheWrite" not in usage
