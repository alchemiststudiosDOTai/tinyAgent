...
class AgentFactory:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(f"Initialized AgentFactory with global rate limit: {self._global_limit}")
    ...
    def create_agent(self, tools: Optional[List[Union[Tool, Callable]]] = None, model: Optional[str] = None) -> 'Agent':
        ...
        """
        # Register tools with factory first
        if tools:
            for tool in tools:
                if isinstance(tool, Tool):
                    ...
                    self.create_tool(
                        name=func_name,
                        description=tool.__doc__ or f"Tool for {func_name}",
                        func=tool
                    )
        ...
        # Create agent with factory reference
        agent = Agent(factory=self, model=model)
        logger.info(f"Created agent with factory containing {len(self._tools)} tools")
    ...
    def execute_tool(self, name: str, *args, **kwargs) -> Any:
        ...
        if tool_limit is not None and tool_limit > 0 and self._call_counts[name] >= tool_limit:
            logger.warning(f"Rate limit exceeded for tool '{name}': {self._call_counts[name]}/{tool_limit}")
